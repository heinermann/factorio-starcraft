...
local config = require('__starcraft__/external/stdlib/config')

local Table = require('__starcraft__/external/stdlib/utils/table')
local Math = require('__starcraft__/external/stdlib/utils/math')
local String = require('__starcraft__/external/stdlib/utils/string')
    ...
    while true do
        ...
        if trace then
            ...
            if (trace.what == 'Lua' or trace.what == 'main') and not ignored[trace.name] then
                local cur = trace.source:gsub('.*__starcraft__/external', '__starcraft__/external'):gsub('.*/Factorio%-Stdlib', '__starcraft__/external')
...
local inspect = require('__starcraft__/external/stdlib/vendor/inspect')
rawset(_ENV, 'inspect', inspect)

-- Defines Mutates
require('__starcraft__/external/stdlib/utils/defines/color')
require('__starcraft__/external/stdlib/utils/defines/anticolor')
require('__starcraft__/external/stdlib/utils/defines/lightcolor')
require('__starcraft__/external/stdlib/utils/defines/time')
...
function STDLIB.create_stdlib_globals(files)
    ...
    for glob, path in pairs(files) do
        rawset(_ENV, glob, require('__starcraft__/external/' .. (path:gsub('%.', '/')))) -- extra () required to emulate select(1)
    end
end
