...
local function format(ingredient, result_count)
    local Item = require('__starcraft__/external/stdlib/data/item')
    local object
...
function Recipe:change_category(category_name)
    if self:is_valid() then
        local Category = require('__starcraft__/external/stdlib/data/category')
...
function Recipe:add_unlock(tech_name)
    if self:is_valid() then
        local Tech = require('__starcraft__/external/stdlib/data/technology')
